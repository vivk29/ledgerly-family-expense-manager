...
export default function LedgerlyInteractionPatch(){
  ...
  useEffect(()=>{
    ...
    const closeTopMenu=()=>{const menu=document.querySelector('.menu');if(menu){const dots=document.querySelector('.dots') as HTMLButtonElement|null;dots?.click();}};
    ...
    const onPop=()=>{if(open){setOpen(false);window.history.pushState({ledgerlyAdd:true},'',window.location.href);}else{closeTopMenu();}};
    document.addEventListener('click',onClick,true);document.addEventListener('keydown',onKey);window.addEventListener('popstate',onPop);
